...
void swap(int *a, int *b)
{
    ...
    *a = (*a) ^ (*b);
    ...
    \begin{solutionorbox}
        ...
        $\equiv \WP(\code{swap},\psi)$ =  $\WP(6-7,\psi[*a \leftarrow *a  \code{^} *b])$

        $\equiv \WP(\code{swap},\psi)$ =  $\WP(6,\psi[*a \leftarrow *a  \code{^} *b][*b \leftarrow *a \code{^} *b])$

        $\equiv \WP(\code{swap},\psi)$ =  $\psi[*a \leftarrow *a  \code{^} *b][*b \leftarrow *a \code{^} *b][*a \leftarrow *a \code{^} *b]$
        ...
        $\equiv \WP(\code{swap},\psi)$ =   $((*a  \code{^} *b) == \old(*b) \wedge *b == \old(*a))[*b \leftarrow *a \code{^} *b][*a \leftarrow *a \code{^} *b]$

        $\equiv \WP(\code{swap},\psi)$ =   $((*a  \code{^} (*a \code{^} *b)) == \old(*b) \wedge (*a \code{^} *b) == \old(*a))[*a \leftarrow *a \code{^} *b]$
    
        $\equiv \WP(\code{swap},\psi)$ =   $((*a \code{^} *b)  \code{^} ((*a \code{^} *b) \code{^} *b)) == \old(*b) \wedge ((*a \code{^} *b) \code{^} *b) == \old(*a)$

        Et en appliquant la règle de simplification du xor, on obtient : 

        $\WP(\code{swap},\psi)$ =   $((*a \code{^} *b)  \code{^} (*a) == \old(*b) \wedge (*a) == \old(*a)$

        $\equiv \WP(\code{swap},\psi)$ =   $(*b) == \old(*b) \wedge (*a) == \old(*a)$

        Puis en retirant les old, on obtient :

        $\WP(\code{swap},\psi)$ =   $(*b) == (*b) \wedge (*a) == (*a)$

        $\equiv \WP(\code{swap},\psi)$ =  $\top$ (ben c'est égal)
 ...
 unsigned int n)
{
    ...
    \begin{solutionorbox}
        ...
        $\equiv \WLP(10-14,I)$ = $\WLP(10-12,I[j \leftarrow j-1][i \ \leftarrow i+1])$

        $\equiv \WLP(10-14,I)$ = $\WLP(10-11,I[j \leftarrow j-1][i \ \leftarrow i+1][t[j] \leftarrow aux])$

        $\equiv \WLP(10-14,I)$ = $\WLP(10,I[j \leftarrow j-1][i \ \leftarrow i+1][t[j] \leftarrow aux][t[i] \leftarrow t[j]])$

        $\equiv \WLP(10-14,I)$ = $I[j \leftarrow j-1][i \ \leftarrow i+1][t[j] \leftarrow aux][t[i] \leftarrow t[j]][aux \leftarrow t[i]]$
    ...
    \begin{solutionorbox}
        ...
        \begin{itemize}
            ...
            $\equiv \WLP(10-14,I_5) = \forall$ integer $k; j-1<k\backslash at(j, Pre) \Rightarrow t[k] == \backslash at(t[j- k + i], Pre)$. \\
    ...
    \begin{solutionorbox}
        ...
        \begin{itemize}
            ...
            \item $\forall$ integer $k; (\backslash at(i, Pre) \leq k<i) \Rightarrow t[k] == \backslash at(t[j- k + i], Pre)$, ce qui est aussi vrai : $\forall$ integer $k; (\backslash at(i, Pre) \leq k<i)$ est équivalent à $\bot$ car i vaut sa valeur avant la boucle, et donc l'implication est vraie.
    
            \item $\forall$ integer $k; j<k\backslash at(j, Pre) \Rightarrow t[k] == \backslash at(t[j- k + i], Pre)$, ce qui est aussi vrai car la partie gauche de l'implication est toujours vraie.
...
\forall integer i,j; 0 <= i < j < n ==> v[i] != v[j];
    ...
    \begin{solutionorbox}
        Une application est bijective si tout élément de son ensemble d'arrivée a un et un seul antécédent. 
        Ici, grâce à $\forall integer i; 0 <= i < n ==> 0 <= v[i] < n$ on voit bien que chaque nombre entre $0$ et $n$ possède une image dans le tableau comprise entre $0$ et $n$.
        La deuxième moitié de la définition nous assure l'unicité de l'image dans l'ensemble d'arrivée, en effet on ne peut pas avoir deux valeurs identiques dans le tableau.
        Cette description du prédicat avec une clause atleast et une clause atmost nous assure la bijection entre les deux ensembles.
    \end{solutionorbox}
...
(unchangedTab{L,M}(t,s,0,n));
    ...
    \begin{solutionorbox}
        \begin{lstlisting}
/*@ predicate isMinPerm{L}(int *t, integer n) =
    $\backslash$ forall integer i ; 0<=i<n-1 ==>
    t[i] < t[i+1] ;

    predicate isMaxPerm{L}(int *t, integer n) =
    $\backslash$ forall integer i ; 0<=i<n-1 ==>
    t[i] > t[i+1] ;
*/
    \end{lstlisting}
...
void initPerm(int *P, unsigned int n);
    ...
    \begin{solutionorbox}
        Soit $\phi = isMinPerm(P,n) \land isPermutation(P,n)$ 
        
        et $I = I_1 \land I_2 = 0 \leq i \leq n \land \forall 0 \leq k < n-i \Rightarrow P[k] ==k $
        $\WLP(initPerm, \phi) \equiv \WLP(5,\WLP(13-16, \phi))$

        $\equiv \WLP(5,I) \equiv 0 \leq n \leq n \land \forall 0 \leq k < n-n \Rightarrow P[k]==k$
    ...
    \begin{solutionorbox}
        On choisit le variant $n-i$, pour prouver que c'est un variant valide, il nous faut montrer qu’il est toujours
        positif à l’entrée de la boucle, et qu’il décroit strictement à chaque tour de boucle.

        $n-i \geq 0$ Car on a pour invariant $0 \leq i \leq n$ (ben voilà).


        $\WLP(14-15,n-i) - \backslash at(n-i,Pre)= n-(i+1) -n + i = n - i-1 -n + i = -1 < 0$, Donc n-i strictement décroissant
    \end{solutionorbox}
...
\begin{lstlisting}
loop invariant \forall integer i ; j<=i<n-1
==> P[i] > P[i+1] ;
\end{lstlisting}
...
int tsp(unsigned int n, int *P)
{
    ...
    \begin{solutionorbox}
        ...
        En effet, tout est prouvé en vert ou vert/orange sauf un assert, qui est en orange.
        Il s'agit de l'assert suivant : 

\begin{lstlisting}
    assert \forall int* t;
    \separated(t+(0..n-1),P+(0..n-1))
    ==> isBiggerPerm{Here,L}(t,P,n)
    ==> isBiggerPerm{L,L}(t,P,n);
\end{lstlisting}
    
        Il nous a néanmoins semblé inutile de se triturer (vraiment) trop la tête, car il nous semble logique que ceci soit vrai.

        En effet, cet assert demande à ce que si un tableau $t$ est séparé de $P$, et que $P$ au temps $L$ est plus grand que $t$ maintenant,
        alors $P$ au temps $L$ est plus grand que $t$ au temps $L$.
...
assert \forall int* t;
\separated(t+(0..n-1),P+(0..n-1))
==> unchangedTab{L,Here}(t,t,0,n);
\end{lstlisting}

        Qui dit que si $t$ est séparé de $P$, alors $t$ au temps $L$ est égal à $t$ maintenant.
        Ainsi, si un tableau $t$ est séparé de $P$, et que $P$ au temps $L$ est plus grand que $t$ maintenant, comme $t$ au temps $L$
        est égal à $t$ maintenant, $P$ au temps $L$ est plus grand que $t$ au temps $L$.
    ...
    \begin{solutionorbox}
        On peut prouver que la fonction termine en utilisant un variant sur $[[1..n]]^n$, et l'ordre utilisé est l'ordre classique sur $\mathbb{N}^n$ :
        c'est à dire $(x_1, x_2, ..., x_n) < (x_1', x_2', ..., x_n')$ si il y a un $i$ compris entre $1$ et $n$ tel que $x_j$ == $x_j'$ pour chaque $j$ strictement plus petit que $i$ ;
        et $x_i < x_i'$ (on a pas de condition sur les indices après $i$). Cet ordre est bien un wqo car l'ensemble considéré est fini.

        En outre, on a bien à chaque tour de boucle un accroissement de $P$ : si on note $P_{pre}$ la valeur de $P$ avant le tour de boucle,
        et $P_{post}$ la valeur après le tour de boucle, comme si on entre dans la boucle on effectue un NextPermutation, et que si on entre dans la boucle, $P_{pre}$ n'est pas maximal,
        on est sûr d'avoir le comportement $notMaxPerm$ qui s'applique, et on a donc $P_{pre} < P_{post}$ : c'est exactement la proprieté de ce comportement.

        En outre, comme on a une permutation, $P$ est toujours dans $[[1..n]]^n$.
        Ainsi, la valeur de $P$ par les tours de boucles effectuent une chaine strictement croissante d'éléments de $[[1..n]]^n$ sur un ordre wqo, et donc comme on a un élément maximal car $[[1..n]]^n$ est fini, on est sûr de terminer la boucle.
    \end{solutionorbox}
